import { POST_ARTICLE, PROFILE_ARTICLES, GET_ARTICLE_BY_ID, UPDATE_ARTICLE, UPDATE_FAIL, DELETE_ARTICLE, DELETE_FAIL } from "../actions/types";
...
export const deleteArticleReducer = (state = initialState, action) =>{
  ...
  switch (action.type) {
  case  DELETE_FAIL:
    alert("error", "Article not deleted", null, null, null);
    return { ...state, article: payload };
  case DELETE_ARTICLE:
    alert("success", "Article successfully Deleted", null, null, "/Profile");
    return { ...state, article: payload };
  default:
    return state;
  }
